...
class UserImageAssign2 extends StatefulWidget {
  String localUser;
  List imageData;

  UserImageAssign2({Key key, @required this.localUser, this.imageData})
      : super(key: key);
  ...
  @override
  _UserImageAssignState2 createState() => _UserImageAssignState2();
}

class _UserImageAssignState2 extends State<UserImageAssign2> {
  ...
  _openGallery() async {
    ...
    setState(() {
      imageFile = pic;
      widget.imageData.add(Image.file(imageFile));
    });
...
//      _uploadedImages.add(imageFile);
  ...
  Widget build(BuildContext context) {
    ...
    void UserChoice(String choice) {
      ...
      if (choice == Constants.close) {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => ImagesListData(loginUser: widget.localUser)));
      }
    }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text("Assign to ${widget.localUser}"),
          leading: InkWell(
            child: Icon(Icons.arrow_back_ios),
            onTap: () {
              Navigator.of(context).push(MaterialPageRoute(
                  builder: (context) => ImagesListData(
                        loginUser: widget.localUser,
                      )));
            },
          ),
          actions: <Widget>[
  ...
  _ShowAssignedImages(BuildContext context) {
    return widget.imageData.length < 1
        ? Center(child: Image.asset("img/no_image.png"))
        : GridView.builder(
            itemCount:
                widget.imageData.length == 0 ? -1 : widget.imageData.length,
            gridDelegate:
                SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2),
            itemBuilder: (BuildContext context, int index) {
              return Card(
                child: Hero(
                  tag: widget.imageData[index],
                  child: Material(
                    child: InkWell(
                      child: GridTile(
                        child: widget.imageData[index],
                      ),
                    ),
                  ),
                ),
